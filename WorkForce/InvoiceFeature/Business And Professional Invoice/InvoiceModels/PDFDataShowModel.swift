...
import Foundation
struct PDFDataShowModel : Codable {
    let status : Int?
    let message : String?
    let data : [PDFData]?
...
struct PDFData : Codable {
    ...
    let product_drtails : [PDFProduct_drtails]?
    ...
    init(from decoder: Decoder) throws {
        ...
        product_drtails = try values.decodeIfPresent([PDFProduct_drtails].self, forKey: .product_drtails)
    }

}
struct PDFProduct_drtails : Codable {
    let id : String?
