//
//  InvoiceAddProductServiceModel.swift
//  WorkForce
//
//  Created by Dharmani Apps on 14/07/23.
//

import Foundation
struct InvoiceAddProductSerModel : Codable {
    let status : Int?
    let message : String?
    let data : InvoiceAddProductData?
    ...
    init(from decoder: Decoder) throws {
        ...
        data = try values.decodeIfPresent(InvoiceAddProductData.self, forKey: .data)
    }

}
struct InvoiceAddProductData : Codable {
    let id : String?
    let user_id : String?
    ...
    let product_drtails : [InvoiceAddproduct_drtails]?
    ...
    init(from decoder: Decoder) throws {
        ...
        product_drtails = try values.decodeIfPresent([InvoiceAddproduct_drtails].self, forKey: .product_drtails)
    }

}
struct InvoiceAddproduct_drtails : Codable {
    let id : String?
