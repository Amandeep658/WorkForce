//
//  CustomerBillingSavedAddressVC.swift
//  WorkForce
//
...
class CustomerBillingSavedAddressVC: UIViewController, UITableViewDelegate,UITableViewDataSource {
    ...
    @IBOutlet weak var selectTableHeightConstraints: NSLayoutConstraint!
    ...
    @IBAction func doneBtn(_ sender: UIButton) {
        if selectedAddress.count == 0{
            ...
            self.navigationController?.backViewController(animated: true, completion: {
                ...
                NotificationCenter.default.post(name: NSNotification.Name.init(rawValue: "dataTransferToCustomerScreen"), object: nil,userInfo: data)
            })
        }
    ...
    func hitSelectSavedAddressApi(){
        ...
        print(param)
        AFWrapperClass.requestPOSTURL(kBASEURL + WSMethods.invoiceCustomerAddressList, params: [:], headers: headers) { [self] (response) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.addressLbl.text = invSavedListArr[indexPath.row].customer_address
        cell.addressLbl2.text = "\(invSavedListArr[indexPath.row].customer_city ?? ""),\(invSavedListArr[indexPath.row].customer_state ?? ""),\(invSavedListArr[indexPath.row].customer_country ?? "")"
        cell.isSelected = false
        cell.selectBtn.setImage(UIImage(named: "circle"), for: .normal)
        DispatchQueue.main.async{
            self.selectTableHeightConstraints.constant = self.selectTableVw.contentSize.height
        }
        return cell
    }
