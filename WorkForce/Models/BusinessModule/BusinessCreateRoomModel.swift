...
import Foundation
struct BusinessCreateRoomModel : Codable {
    let status : Int?
    let message : String?
    let room_id : String?
    let loginProfile : String?
    let other_id : String?
    let user_detail : BusinessRoomCreateUser_detail?
    ...
    init(from decoder: Decoder) throws {
        ...
        user_detail = try values.decodeIfPresent(BusinessRoomCreateUser_detail.self, forKey: .user_detail)
    }

}
struct BusinessRoomCreateUser_detail : Codable {
    let user_id : String?
    let username : String?
