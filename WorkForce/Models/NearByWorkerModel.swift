...
import Foundation
struct NearByWorkerModel : Codable {
    let status : Int?
    let message : String?
...
struct NearWorkerData : Codable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case isLike = "isLike"
        case worker_details = "worker_details"
    ...
    init(from decoder: Decoder) throws {
        ...
        worker_details = try values.decodeIfPresent(Worker_details.self, forKey: .worker_details)
    }

}

struct Worker_details : Codable {
    let user_id : String?
    let username : String?
    let company_name : String?
    let mobile_no : String?
