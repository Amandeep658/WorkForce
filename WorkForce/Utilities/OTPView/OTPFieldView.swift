...
@IBDesignable class OTPFieldView: UIView {
    ...
    public var defaultBackgroundColor: UIColor = UIColor(hexString: "F5F6F7")
    ...
    fileprivate func getOTPField(forIndex index: Int) -> OTPTextField {
        ...
        if hasOddNumberOfFields {
            ...
            // Calculate from middle each fields x and y values so as to align the entire view in center
            fieldFrame.origin.x = (CGFloat(index) * (fieldSize + separatorSpace))
                
//                bounds.size.width / 2 - (CGFloat(fieldsCount / 2 - index) * fieldSize + CGFloat(fieldsCount / 2 - index - 1) * separatorSpace + separatorSpace / 2)
        }
